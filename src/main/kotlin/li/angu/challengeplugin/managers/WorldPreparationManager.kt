...
class WorldPreparationManager(private val plugin: ChallengePluginPlugin) {
    ...
    fun countAvailableWorlds(): Int {
        return worldsFolder.listFiles()?.filter { it.isDirectory && !it.name.endsWith("nether") && !it.name.endsWith("the_end") }?.size ?: 0
    }
    ...
    fun getWorldForChallenge(challengeId: UUID): String? {
        val availableWorlds = worldsFolder.listFiles()?.filter { it.isDirectory && !it.name.endsWith("_nether") && !it.name.endsWith("_the_end") } ?: return null
